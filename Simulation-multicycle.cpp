#include "Simulation-multicycle.h"
using namespace std;
    ...
    while(PC<end) {
        ...
        switch(itype){
            ...
            case IOP:
                ...
                MEM_WB = MEM_WB_old;
                WB();
                if(last_inst_div && this_inst_rem)
                    clock_count += 1;
                else
                    clock_count += 1 + alu_clock_cycles;
                break;
            case MEMR:
                EX();
                EX_MEM = EX_MEM_old;
...
inst_type ID()
{
    ...
    if(newPCSrc==ALUOUT){//JAL/BEQ
        target = temp_PC + ext_signed(EXTsrc,EXTop,immlen);
        //newPC = ALUout;
    }
    else if(newPCSrc==ALUOUT_0){//JALR
        target = (reg[rs] + ext_signed(EXTsrc,EXTop,immlen)) & (-2);
        //newPC = ALUout & (-2);
    }
